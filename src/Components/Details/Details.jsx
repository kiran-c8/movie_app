...
const Details = () => {
  ...
  return (
    <div style={{ background: '#010003', height: '100%' }}>
      {
        Object.keys(movie).length !== 0 && credits.length !== 0 && similarMovies.length !== 0 ? (
          <>
            <div className='banner' style={{ backgroundImage: `url('${imageUrl + movie.backdrop_path}')` }}></div>

            {/* Movie details  */}
            <div className='movie-content'>
              <div className="movie-content_poster">
                <div className="movie-content_poster__img" style={{ backgroundImage: `url('${imageUrl + movie.poster_path}')` }}></div>
              </div>
              <div className="movie-content_info">
                ...
                <div className="movie-content_info__genre">
                  {
                    movie.genres && movie.genres.map(item => {
                      return (
                        <span className='movie-content_info__genre__item'>{item ? item.name : ''}</span>
                      )
                    })
                  }
                </div>
                ...
                <div className="movie-content_info__rating">
                  ...
                  <FontAwesomeIcon icon={faStar} className='star-icon' color="#f5c518" />
                  <span>{movie.vote_average}/10</span>
                </div>
                <div className='movie-content_button' onClick={() => navigate(`/details/video?movieId=${movie.id}`)}>
                        ...
                        credits.map(actor => {
                          return (
                            <div className="cast-section_card__image">
                              <img src={`${imageUrl + actor.profile_path}`} alt="" className='cast-section_card__image__poster' />
                  ...
                  similarMovies.map(item => {
                    return (
                      <div className="similar-movies-container_image">
                        <img src={`${imageUrl + item.poster_path}`} alt="" className='similar-movie_poster' />
                        <div className="similar-movies-container_image-overlay" onClick={() => navigate(`/details?movieId=${item.id}`)}>
                          <h5>{item.title}</h5>
                        </div>
                      </div>
                    )
